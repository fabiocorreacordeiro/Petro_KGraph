...
def main():
    ...
    parser.add_argument('--rank',
                        ...
                        required=False,
                        help=".")
    parser.add_argument('--master_add',
                        type=str,
                        default='0.0.0.0',
                        required=False,
                        help=".")
    ...
    parser.add_argument('--process',
                        ...
                        default=1,
                        required=False,
                        help="numero de processos que serão enviados")
    

    args = parser.parse_args()
    
    if args.local_rank != -1:
        os.environ['MASTER_ADDR'] = args.master_add              #
    ...
    if args.do_train:
        ...
        train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=int(args.train_batch_size/args.process))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=int(args.eval_batch_size/args.process))
        model.eval()
        ...
        for input_ids, input_mask, segment_ids, label_ids,valid_ids,l_mask in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            for sentence in logits.tolist():
                y_pred.append([label_map[1] if int(elem) in [0,10,11] else label_map[elem] for elem in sentence])
                

            if label_ids.tolist() != None:
                for idx in label_ids.tolist():
                    y_true.append([ label_map[1] if int(elem) in [0,10,11] else label_map[elem] for elem in idx]) 
